...
- Enhance: ZALIPのホームを背景ビジュアル付きの作品スライダーにし、フィード、ライブラリ、カタログ、モバイルナビゲーションを整理
- Fix: ZALIPのカタログ・ライブラリ・購読画面に残っていた固定文言を翻訳キーへ移行
- Fix: ノートのホバー背景でテーマの透明度を保持し、タッチ端末で強いハイライトが残らないように
- Fix: ZALIPモバイル作品ページの文字・アイコン・操作領域を拡大し、ライトテーマの作品カードから白いグラデーションを削除
- Fix: ZALIPの作品・エピソードカードの横スクロールで、操作後に端へ吸着しないように
- Feat: ZALIPのカタログを独立ページにし、複数の作品種別・ジャンル、公開年、検索語を組み合わせるフィルタと並び替えを追加
- Enhance: ZALIPカタログに作品種別とジャンルを組み合わせるフィルタ、作品ページに10段階の個人評価を追加
- Feat: ZALIPのカタログ、投稿、プロフィールを横断するタブ付き検索を追加
- Enhance: ZALIPにシステム追従を含むライト・ダークテーマと4色のアクセントパレット、カタログの並び替え・ジャンルフィルタを追加
- Enhance: ZALIPのトップページに、新着エピソードをポスターで探せる横スクロールのショーケースを追加
- Enhance: ZALIP作品ページでプレイヤーと公式トレーラーを同じ画面内で切り替え、ライブラリのステータス選択を追加
- Enhance: ZALIP作品ページのアクションとプレイヤータブを整理
- Enhance: ZALIP作品ページをポスターと背景画像を統合したヒーローレイアウトに刷新
- Enhance: ZALIP作品ページとモバイル下部ナビゲーションを小さな画面向けに最適化
- Fix: ZALIP作品ページのAllohaプレイヤーが、選択したシーズンとエピソードを開くように修正
- Enhance: ZALIP作品ページを、左側のポスターとアクション、プレイヤー中心のレイアウトに再構成
- Feat: ZALIP向けの映画、フィード、ライブラリ、購読、プロフィールに絞ったナビゲーションを追加
- Feat: ZALIP作品ページで作品と各エピソードのコメントをページ遷移なしに切り替えられるように
- Feat: ZALIPの新着エピソードをMisskeyの通知画面とWeb Pushから開けるように
- Feat: ZALIPエディターからシーズンとエピソードを手入力できるように
- Feat: ZALIP作品ページからお気に入り、10段階評価、視聴済みエピソード数を保存できるように
- Feat: ZALIP作品ページでTMDBの背景画像と公式トレーラーを表示できるように
- Feat: ZALIPライブラリでステータス・お気に入り・評価の集計を表示できるように
- Enhance: ZALIP作品ページの個人評価を10段階の星と評価文を備えた専用ダイアログに刷新
- Enhance: ZALIP全体をOngaku型の3カラム構成、コンテンツカード、検索、レスポンシブナビゲーションに刷新
- Enhance: ZALIPのトップページをOngaku型オンボーディングスライダーに置き換え、作品とエピソードのカードをタッチ・マウス操作で横スクロール可能に

- Fix: ZALIPのカードクリックとマウスドラッグ、評価ダイアログの保存・配色・高さ、モバイル作品ページのポスター・ボタン幅とアカウントアバターを修正し、ホームフィードと認証・ウィジェットの導線を改善
- Enhance: ZALIPのフィードを2段の投稿ヘッダーと読みやすい作品カード・操作ボタンに刷新し、投稿本文のクリックでコメント付きの詳細を開けるように

- Enhance: ZALIPの中央領域を拡張し、ホームとタイムラインのフィード、デスクトップヘッダー、カタログのレスポンシブフィルターとアクセント配色を統一
- Enhance: ZALIPのチャット・通知・ブックマークを共通の軽量なレイアウトに統一し、選択済みリアクションの色付き背景を削除
...
- Fix: 未ログインのユーザー検索でロールポリシーの確認が失敗する問題を修正
- Enhance: ZALIP作品一覧APIで複数の種別・ジャンル、公開年、タイトル検索を同時に絞り込めるように
- Enhance: ZALIPカタログとタイトル検索APIで映画・シリーズ・アニメ・アニメーション種別の絞り込みに対応
- Enhance: 公開済みZALIP作品から完全なジャンル一覧を返すカタログAPIを追加
- Feat: ログインしたユーザーがZALIP作品・エピソードのコメントスレッドを開始できるAPIを追加
- Feat: 購読中のZALIP作品に新エピソードが追加された時の耐障害性を持つ通知配信を追加
- Feat: ZALIP作品のローカルシーズンとエピソードを管理者が追加できるAPIを追加
- Fix: VDSのDNS設定によってTMDBインポートがループバックへ接続してしまう問題を修正
- Enhance: ZALIP作品一覧APIで一度に指定できるジャンルを24件に拡張
